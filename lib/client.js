...
`;
    ...
    function readLegacyFontPreference() {
      ...
      try {
        return normalizeFontFamily(
          localStorage.getItem(LEGACY_FONT_STORAGE_KEY),
        );
      } catch {
    ...
    function fontPreferenceOf(settingsStore) {
      if (settingsStore === undefined) return readLegacyFontPreference();
      return normalizeFontFamily(
        settingsStore.getSnapshot().value?.[FONT_SETTINGS_FIELD],
      );
    }
    ...
    function FontSettingsRow({
      ...
      const rankedFonts = useMemo(
        () => rankFontCandidates(fonts, fontFamily),
        [fonts, fontFamily],
      );
      ...
      return h(
        "div",
        { className: "dcr-fontSetting" },
        h(
          "div",
          { className: "dcr-fontSettingText" },
          h("div", { className: "dcr-fontSettingTitle" }, "变更代码字体"),
          h(
            "div",
            { className: "dcr-fontSettingHint" },
            fontError ??
              (fontFamily === committedFont.current
                ? `当前已应用：${committedFont.current}`
                : `待选择字体 · 当前已应用：${committedFont.current}`),
          ),
        ),
        h(
          "div",
          { className: "dcr-fontControl" },
          h(Menu, {
    ...
    const HIGHLIGHT_KEYS = HIGHLIGHT_FIELDS.flatMap(([, fields]) =>
      fields.map(([key]) => key),
    );
    ...
    function readHighlightOverrides() {
      if (typeof localStorage === "undefined") return emptyHighlightOverrides();
      try {
        return (
          sanitizeHighlightOverrides(
            localStorage.getItem(HIGHLIGHT_STORAGE_KEY),
          ) ?? emptyHighlightOverrides()
        );
      } catch {
    ...
    function writeLegacyHighlightOverrides(overrides) {
      ...
      try {
        localStorage.setItem(
          ...
          serializeHighlightOverrides(overrides),
        );
      } catch {
    ...
    function writeHighlightOverrides(settingsStore, overrides) {
      ...
      if (
        ...
        return;
      try {
        const pending = settingsStore.set(
          HIGHLIGHT_SETTINGS_FIELD,
          serializeHighlightOverrides(overrides),
        );
    ...
    function settingsHighlightOverrides(settingsStore) {
      if (
        ...
        return undefined;
      return sanitizeHighlightOverrides(
        settingsStore.getSnapshot().value?.[HIGHLIGHT_SETTINGS_FIELD],
      );
    }
    ...
    function paletteStyle(palette) {
      return Object.fromEntries(
        HIGHLIGHT_KEYS.map((key) => [`--dcr-${key}`, palette[key]]),
      );
    }
    ...
    function createHighlightStore(theme, settingsStore) {
      let overrides =
        settingsHighlightOverrides(settingsStore) ?? readHighlightOverrides();
      let revision = 0;
    ...
    function HighlightColorRow({
      ...
      return h(
        ...
        h("input", {
          ...
          onChange: (event) => commit(event.target.value),
          onBlur: () => {
            if (!HEX_COLOR.test(draft)) setDraft(color);
          },
        }),
        h(
          "button",
    ...
    function HighlightPreview({ palette }) {
      ...
      const code = (kind, number, marker, children) =>
        h(
          "div",
          {
      ...
      return h(
        ...
        code(
          ...
          h(
            ...
            h(
              "span",
              { style: { color: "var(--dcr-syntax-keyword)" } },
              "const",
            ),
            " ",
            h(
              "span",
              { style: { color: "var(--dcr-syntax-variable)" } },
              "message",
            ),
            " ",
            ...
            h(
              "span",
              { style: { color: "var(--dcr-syntax-string)" } },
              '"ready"',
            ),
            h(
              "span",
              { style: { color: "var(--dcr-syntax-punctuation)" } },
              ";",
            ),
          ),
        ),
        code(
          ...
          h(
            "span",
            { style: { color: "var(--dcr-syntax-comment)" } },
            "// old implementation",
          ),
        ),
        ...
        code(
          ...
          h(
            ...
            h(
              "span",
              { style: { color: "var(--dcr-syntax-function)" } },
              "render",
            ),
            h(
              "span",
    ...
    function HighlightSettingsContent({ highlightStore, disabled = false }) {
      ...
      return h(
        ...
        h(
          "div",
          { className: "dcr-highlightToolbar" },
          h(
            "div",
            {
        ...
        HIGHLIGHT_FIELDS.map(([section, fields]) =>
          h(
            "section",
            { className: "dcr-paletteSection", key: section },
            h("h3", null, section),
            h(
              "div",
              { className: "dcr-colorGrid" },
    ...
    function HighlightSettingsTab({ highlightStore }) {
      return h(
        "main",
        { className: "dcr-highlightSettings" },
        h("h2", null, "代码高亮"),
        h(
          "p",
          { className: "dcr-highlightIntro" },
          "配色方案 · 可分别编辑亮色和暗色界面使用的颜色",
        ),
    ...
    function CodeReviewPluginCard({ highlightStore, settingsStore }) {
      ...
      const fontFamily = normalizeFontFamily(
        settingsSnapshot.value?.[FONT_SETTINGS_FIELD],
      );
      const writable =
      ...
      return h(
        ...
        h(
          ...
          h(
            "span",
            { className: "dcr-settingsHeadText" },
            h("span", { className: "dcr-settingsTitle" }, "dsh-code-review"),
            h(
              "span",
              { className: "dcr-settingsStatus" },
              "代码变更、字体和代码高亮设置 · ",
              status,
            ),
          ),
        ...
        expanded &&
          h(
            "div",
            { className: "dcr-settingsContent" },
            h(FontSettingsRow, { fontFamily, setFont, disabled: !writable }),
            h(
              "section",
              { className: "dcr-pluginHighlightSection" },
              h("h3", null, "代码高亮"),
              h(
                "p",
                { className: "dcr-highlightIntro" },
        ...
        window.requestAnimationFrame(() =>
          window.requestAnimationFrame(() => {
            ...
            const availableCenter =
              frame.getBoundingClientRect().width - leftWidth - snapshot.width;
            if (
              availableCenter < 640 &&
    ...
    function finalizeTreeDirectory(raw, name, key, compress = true) {
      let node = {
        ...
        name,
        key,
        children: [...raw.children.entries()].map(([childName, child]) =>
    ...
    function buildFileTree(files) {
      ...
      const roots = [...groups.entries()].map(([key, group]) =>
        finalizeTreeDirectory(group, group.label, key || "workspace", false),
      );
    ...
    async function requestReview(sessionId, options = {}) {
      const response =
        options.action === "undo"
          ...
          : await fetch(
              `${API_PATH}?sessionId=${encodeURIComponent(sessionId)}`,
              {
    ...
    function useReviewState(sessionId, useSession, poll = false) {
      const signal = useSession(
        (snapshot) =>
          `${snapshot.chat.order.length}:${snapshot.running ? 1 : 0}:${snapshot.turnEnds.size}`,
      );
    ...
    function HighlightedText({ runs, text }) {
      ...
      return h(
        ...
        null,
        runs.map((run, index) =>
          h("span", { key: index, style: { color: run.color } }, run.text),
        ),
      );
    }
    ...
    function LegacyDiffView({ file }) {
      ...
      return h("section", { id: fileAnchorId(file.path), className: "dcr-unified", "aria-label": `${file.path} 的代码变更` },
        ...
        h("div", { className: "dcr-unifiedBody" }, blocks.map((row) => h("div", {
          className: `dcr-diffRow${row.kind === "add" ? " dcr-diffRowAdd" : " dcr-diffRowDel"}`,
          key: row.key,
        },
          h("span", { className: "dcr-lineNo", "aria-hidden": true }, ""),
          h("span", { className: "dcr-lineNo" }, row.line),
          h("span", { className: "dcr-marker", "aria-hidden": true }, row.kind === "add" ? "+" : "-"),
          h("code", { className: "dcr-codeLine" }, h(HighlightedText, { runs: row.runs, text: row.text })),
        ))),
      );
    }
    ...
    function UnifiedDiffView({ file }) {
      const highlighted = useMemo(() => {
        ...
        const lang = engine?.languageFromPath(file.path);
        return {
          old: engine?.highlightLines(
            typeof file.oldText === "string" ? file.oldText : "",
            lang,
          ),
          next: engine?.highlightLines(
            typeof file.newText === "string" ? file.newText : "",
            lang,
          ),
        };
      ...
      return h(
        ...
        h(
          "header",
          { className: "dcr-unifiedHeader" },
          h(
            "span",
            { className: "dcr-unifiedPath", title: file.path },
            file.path,
          ),
          h(Stats, { added: file.added, removed: file.removed }),
        ),
        h(
          "div",
          { className: "dcr-unifiedBody" },
          file.unified.sections.map((section, sectionIndex) =>
            section.kind === "gap"
              ? h(
                  "div",
                  { className: "dcr-gap", key: `gap-${sectionIndex}` },
                  `${section.count} 行未修改`,
                )
              : h(
                  React.Fragment,
                  { key: `hunk-${sectionIndex}` },
                  section.rows.map((row, rowIndex) => {
                    const line = row.kind === "del" ? row.oldLine : row.newLine;
                    const runs =
                      row.kind === "del" ? highlighted.old : highlighted.next;
                    return h(
                      "div",
                      {
                      ...
                      h(
                        "span",
                        { className: "dcr-marker", "aria-hidden": true },
                        row.kind === "add"
                          ? "+"
                          : row.kind === "del"
                            ? "-"
    ...
    function DiffFiles({ files }) {
      return h(
        ...
        files.map((file) =>
          h(
            "div",
            { className: "dcr-inlineFile", key: file.path },
            h(UnifiedDiffView, { file }),
          ),
        ),
      );
    }
    ...
    function LargeDiffNotice() {
      return h(
        "div",
        { className: "dcr-largeNotice", role: "status" },
        h(IconWarningOutline16, { size: 15 }),
    ...
    function UndoControls({
      ...
      return h(
        ...
        h(
          ...
          h(IconRefreshOutline16, { size: 14 }),
          pending ? "撤销中" : confirming ? "确认撤销" : "撤销",
        ),
        confirming &&
          h(
            "button",
    ...
    function ReviewSummaryNode({
      ...
      const { state, setState, error, setError } = useReviewState(
        sessionId,
        useSession,
      );
      const [expanded, setExpanded] = useState(false);
      ...
      return h(
        ...
        h(
          "div",
          { className: "dcr-summary" },
          h(
            "div",
            { className: "dcr-summaryTop" },
            h(
              "span",
              { className: "dcr-summaryIcon" },
              h(IconCodeOutline16, { size: 16 }),
            ),
            h(
              "div",
              { className: "dcr-summaryText" },
              h(
                "div",
                { className: `dcr-summaryTitle${undone ? " dcr-undone" : ""}` },
                undone
                  ? `已撤销 ${files.length} 个文件的变更`
                  : `已编辑 ${files.length} 个文件 · ${changedLines} 行`,
              ),
              h(
                "div",
                { className: "dcr-summarySub" },
                undone ? "文件已恢复到本轮修改前" : `第 ${turn} 轮`,
                " ",
                h(Stats, { added: stats.added, removed: stats.removed }),
              ),
            ),
            h(
              "div",
              { className: "dcr-summaryActions" },
              h(UndoControls, {
                sessionId,
                turn,
                disabled: !canUndo,
        ...
        expanded &&
          h(
            "div",
            { className: "dcr-inlineDiff" },
            isLargeDiff(changedLines) && h(LargeDiffNotice),
            h(DiffFiles, { files: inlineFiles }),
          ),
      );
    }
      ...
      useLayoutEffect(() => {
        ...
        const measure = () => {
          ...
          const actualWidth = Math.min(
            widthRef.current,
            Math.max(0, frameRect.width - leftWidth),
          );
    ...
    function TurnPicker({ state, selectedTurn, onSelect }) {
      ...
      const selected =
        turns.find((turn) => turn.turn === selectedTurn) ?? turns[0];
      const label = (turn) =>
        `${turn.turn === state.latestTurn ? "上一轮" : `第 ${turn.turn} 轮`} · ${turn.files.length} 个文件 · ${turn.changedLines ?? turn.added + turn.removed} 行${turn.undone ? " · 已撤销" : ""}`;
      ...
      const anchor = h(
        ...
        h("span", { className: "dcr-turnPickerLabel" }, label(selected)),
    ...
    function FileTreeNode({
      ...
      if (node.type === "file") {
        return h(
          ...
          h(
            "span",
            { className: "dcr-treeIcon" },
            h("span", { className: "dcr-treeFileIcon" }),
          ),
          h("span", { className: "dcr-treeLabel" }, node.name),
          h(
            "span",
            { className: "dcr-treeStats" },
      ...
      return h(
        ...
        h(
          ...
          h(
            "span",
            { className: "dcr-treeChevron" },
            isOpen
              ? h(IconChevronDownOutline14, { size: 13 })
              : h(IconChevronRightOutline14, { size: 13 }),
          ),
          h(
            "span",
            { className: "dcr-treeIcon" },
    ...
    function SidebarFilePane({ files, selectedPath, onSelect }) {
      ...
      return h(
        "aside",
        { className: "dcr-filePane", "aria-label": "变更文件" },
        h(
          "div",
          { className: "dcr-fileSearchWrap" },
          h("input", {
    ...
    function FilePaneResizeHandle({ mainRef, width, onResize }) {
      ...
      const resizeFromPointer = (event) => {
        ...
        onResize(
          filePaneWidthFromPointer(rect.left, rect.right, event.clientX),
        );
      };
    ...
    function ChangesSidebar({
      ...
      const { state, setState, error, setError, refresh } = useReviewState(
        sessionId,
        useSession,
        true,
      );
      const highlightSnapshot = useSyncExternalStore(
        highlightStore.subscribe,
        highlightStore.getSnapshot,
        highlightStore.getSnapshot,
      );
      const reviewStyle = paletteStyle(
        paletteFor(highlightSnapshot.overrides, highlightSnapshot.scheme),
      );
      const [selectedTurn, setSelectedTurn] = useState(
        navigation?.turn ?? null,
      );
      const [selectedPath, setSelectedPath] = useState(
        navigation?.path ?? null,
      );
      const [filePaneWidth, setFilePaneWidth] = useState(
        readFilePaneWidthPreference,
      );
      ...
      const turnState =
        state?.turns.find((turn) => turn.turn === selectedTurn) ?? null;
      ...
      const changedLines =
        turnState?.changedLines ??
        (turnState?.added ?? 0) + (turnState?.removed ?? 0);
      const renderedFiles = filesForReview(
        files,
        changedLines,
        selectedFile?.path,
      );
      const selectFile = (path) => {
        ...
        if (isLargeDiff(changedLines) || typeof window === "undefined") return;
        const scroll = () => scrollToFile(path);
        if (typeof window.requestAnimationFrame === "function")
          window.requestAnimationFrame(scroll);
        else queueMicrotask(scroll);
      };
      const visibleFilePaneWidth =
        mainWidth > 0 ? Math.min(filePaneWidth, mainWidth) : filePaneWidth;
      if (state === null && error === null)
        return h(
          "div",
          { className: "dcr-review", style: reviewStyle },
          h("div", { className: "dcr-empty" }, "正在读取变更…"),
        );
      return h(
        "section",
        {
        ...
        h(
          "header",
          { className: "dcr-reviewHeader" },
          h(
            "div",
            { className: "dcr-reviewHeaderTop" },
            h(
              "div",
              { className: "dcr-titleGroup" },
              h("h2", { className: "dcr-title" }, "变更"),
              turnState &&
                h("span", { className: "dcr-lineTotal" }, `${changedLines} 行`),
              turnState &&
                h(Stats, {
        ...
        files.length === 0
          ...
          : h(
              ...
              h(
                "main",
                { className: "dcr-diffPane" },
                h(DiffFiles, { files: renderedFiles }),
              ),
              h(SidebarFilePane, {
                files,
    ...
    function ChangesSidebarPanel({ sidebarController, width, ...props }) {
      ...
      const resizeFromPointer = (event) => {
        ...
        if (frame === null || frame === undefined) return;
        const frameRect = frame.getBoundingClientRect();
        sidebarController.resize(
          sidebarWidthFromPointer(
            frameRect.right,
            frameRect.left + frameSidebarWidth(frame),
            event.clientX,
          ),
        );
      };
    ...
    function apply(ctx) {
      ...
      const openPath = (path) => {
        const workspaces =
          typeof ctx.get === "function" ? ctx.get("workspaces") : undefined;
      ...
      ctx.effect(() =>
        typeof ctx.on === "function"
          ? ctx.on("theme/change", (snapshot) =>
              highlightStore.setScheme(snapshot.active.colorScheme),
            )
          : undefined,
      );
      ctx.effect(() => {
        const applyStoredFont = () =>
          applyFontPreference(fontPreferenceOf(settingsStore));
        applyStoredFont();
