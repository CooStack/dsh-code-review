...
function myersOperations(before, after) {
  ...
  for (let distance = 0; distance <= max; distance += 1) {
    ...
    for (let diagonal = -distance; diagonal <= distance; diagonal += 2) {
      ...
      let backY = m;
      for (
        let backDistance = trace.length - 1;
        backDistance >= 0;
        backDistance -= 1
      ) {
...
export function buildUnifiedDiff(oldText, newText, contextLines = 3) {
  const operations = myersOperations(
    contentLines(oldText),
    contentLines(newText),
  );
  let oldLine = 1;
  let newLine = 1;
  let added = 0;
...
function upsertRecord(state, next) {
  ...
  if (index < 0) {
    ...
    state.records.sort(
      (a, b) => a.ordinal - b.ordinal || a.key.localeCompare(b.key),
    );
    state.revision += 1;
    return true;
  }
  ...
  const finalCandidates =
    expectedAfter === undefined
      ? remaining.filter(
          (record) =>
            !remaining.some(
              (other) => other !== record && other.before === record.after,
            ),
        )
  ...
  return [...files.values()]
    ...
    .map((file) => {
      ...
      return {
        ...
        concurrentConflict:
          file.records.every((record) => record.hasSnapshot) && !chain.ok,
        added: stats.added,
...
export function buildReviewState(state, running = false) {
  ...
  const allStats = allFiles.reduce(
    (sum, file) => ({
      added: sum.added + file.added,
      removed: sum.removed + file.removed,
    }),
  ...
  return {
    ...
    running,
    latestTurn:
      turns.find((turn) => !turn.undone)?.turn ?? turns[0]?.turn ?? null,
    turns,
    all: {
      files: allFiles,
...
export function createLedgerRuntime(ctx, storageRoot = STORAGE_ROOT) {
  ...
  const persist = async (state) => {
    ...
    await atomicWriteJson(
      join(storageRoot, sessionFileName(state.sessionId)),
  ...
  const schedulePersist = (state) => {
    ...
    const next = previous
      ...
      .catch((error) => {
        console.error(
          `[dsh-code-review] failed to persist ${state.sessionId}:`,
          error,
        );
      });
    ...
    for (const entry of entries) {
      ...
      try {
        const parsed = JSON.parse(
          await readFile(join(storageRoot, entry.name), "utf8"),
        );
        if (
          !isObject(parsed) ||
        ...
        console.error(
          `[dsh-code-review] ignored unreadable ledger ${entry.name}:`,
          error,
        );
      }
    }
  ...
  const ownerStates = (sessionId) =>
    [...states.values()].filter(
      (state) =>
        state.sessionId === sessionId || state.ownerSessionId === sessionId,
    );

  const projectedOwnerState = (sessionId) => {
    const owned = ownerStates(sessionId);
    const records = owned.flatMap((state) =>
  ...
  const ownerRunning = (sessionId) => {
    for (const agent of ctx.agents.list()) {
      ...
      if (
        (agent.id === sessionId || state.ownerSessionId === sessionId) &&
  ...
  const reviewState = (sessionId) => {
    ...
    return buildReviewState(
      projectedOwnerState(sessionId),
      ownerRunning(sessionId),
    );
  };
  ...
  const undoTurn = async (sessionId, turn) => {
    ...
    const sandboxPolicy = ctx.sandboxPolicy.resolve(
      agent === undefined ? undefined : { session: agent.session },
    );
    const entries = ownerStates(sessionId).flatMap((state) =>
      state.records
        .filter(
          (record) =>
            (state.sessionId === sessionId
    ...
    if (records.some((record) => !record.hasSnapshot)) {
      throw new HttpError(
        409,
        "这一轮在插件启用前完成，只有 diff 记录，没有可恢复快照",
      );
    }
    ...
    for (const [path, changes] of grouped) {
      const cwd =
        changes.find((record) => record.cwd !== undefined)?.cwd ??
      ...
      if (!chain.ok) {
        throw new HttpError(
          409,
          `无法撤销 ${path}：子智能体并发修改无法形成唯一快照链`,
          { path, reason: chain.reason },
        );
      }
      prepared.push({
        path,
    ...
    try {
      ...
      for (const item of applied.reverse()) {
        try {
          ...
          console.error(
            `[dsh-code-review] rollback after failed undo also failed for ${item.path}:`,
            rollbackError,
          );
        }
      }
    ...
    await Promise.all(
      [...touchedStates].map((state) => schedulePersist(state)),
    );
  ...
  return {
    ...
    flush: (sessionId) =>
      Promise.all(
        ownerStates(sessionId).map(
          (state) => persistTails.get(state.sessionId) ?? Promise.resolve(),
        ),
      ),
  };
}
...
function configRouteHandler(scope) {
  return async (req, res) => {
    try {
      ...
      assertMutationRequest(req);
      const body = await readJsonBody(req);
      if (!isObject(body)) throw new HttpError(400, "配置必须是对象");
